    ...
    <>
      ...
      <div className="container mx-auto p-4">
        <div className="flex justify-end mb-4">
          <select value={newsType} onChange={handleChangeNewsType} className="p-2 border rounded">
        ...
        <div className="flex justify-center mt-8">
          {Array.from({ length: Math.ceil(filteredNews.length / newsPerPage) }, (_, i) => (
            <button
              key={i + 1}
              onClick={() => paginate(i + 1)}
              className={`px-4 py-2 border rounded ${currentPage === i + 1 ? 'bg-blue-500 text-white' : 'bg-white text-black'}`}
            >
              {i + 1}
            </button>
          ))}
